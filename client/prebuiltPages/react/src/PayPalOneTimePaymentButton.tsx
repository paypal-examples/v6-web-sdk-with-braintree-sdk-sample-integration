...
export const PayPalOneTimePaymentButton: React.FC<UseBraintreePayPalOneTimePaymentSessionProps> = (props) => {
  const { isPending, handleClick } =
    useBraintreePayPalOneTimePaymentSession(props);

  return (
    <paypal-button onClick={() => handleClick()} disabled={isPending}></paypal-button>
  );
}