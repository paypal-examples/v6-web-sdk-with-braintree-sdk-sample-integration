  ...
  const paypalPaymentSession =
    paypalCheckoutV6Instance.createCheckoutWithVaultSession({
      ...
      async onApprove(data) {
        ...
        renderAlert({
          type: "success",
          message: `Order successfully captured and payment method vaulted: ${JSON.stringify(data)}`,
        });
        console.log("Capture and vault result", orderData);
      },
      onCancel(data) {
        renderAlert({
          type: "warning",
...
async function completePaymentAndVault(paymentMethodNonce) {
